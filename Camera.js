class Camera {
    ...
    scrollDrawing(_scroll) {
        ...
        if (_scale > 0) {
            scene._left = -width / _scale
            scene._right = width / _scale
            scene._bottom = -height / _scale
            scene._top = height / _scale
        }
        else {
            _scale = abs(_scroll)
        }
    ...
    useLoop() {
        if (buleanLoop == false) {
            ...
            angle = 0
            scene.x = (height / 2) / (PI / 6) * sin(angle)
            scene.z = (height / 2) / (PI / 6) * cos(angle)
            loop()
        } else {
            buleanLoop = false
